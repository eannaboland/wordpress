...
class UpdraftCentral_Licence_Manager {
	...
	public function is_slot_available() {
		if ($this->no_licences_required) return true;
		$licences_in_use = $this->how_many_licences_in_use();
		$licences_available = $this->how_many_licences_available();

		return $licences_available < 0 || $licences_in_use < $licences_available;
	}
	...
	public function how_many_licences_in_use() {
		if (!is_array($this->user->sites)) return 0;

		return count($this->user->sites);
	}
