...
function get_coordinates($data) {
	...
	$tablename = $wpdb->prefix.'map_locations';

	//count the number of pitches per club
	$pitchcount = $wpdb->get_results( "SELECT count(*) FROM $tablename WHERE location_address LIKE '%$hometeam%' ", OBJECT );
	...
	//count the number of pitches per club
	if ($numpitches == 1) {
		$pitchdata = $wpdb->get_results( "SELECT location_title,location_latitude,location_longitude FROM $tablename WHERE location_address LIKE '%$hometeam%'", OBJECT );
	...
	//count the number of pitches per club
	elseif($numpitches> 1) {  $pitchdata = $wpdb->get_results( "SELECT location_title,location_latitude,location_longitude 
 FROM $tablename
